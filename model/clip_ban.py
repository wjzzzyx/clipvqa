...
from .modules import BiAttention, BiResNet
...
class ClipBAN(nn.Module):
    ...
    def __init__(self, dataset, cfg):
        ...
        if cfg.TRAIN.CLIP_TYPE == 'origin':
            ...
            self.visual_projection2 = nn.Linear(cfg.TRAIN.VISION.POOL_DIM, cfg.TRAIN.VISION.HID_DIM, bias=False)
        ...
        self.close_att = BiAttention(cfg.TRAIN.VISION.POOL_DIM, cfg.TRAIN.QUESTION.HID_DIM, cfg.TRAIN.QUESTION.HID_DIM, cfg.TRAIN.ATTENTION.GLIMPSE)
        self.close_resnet = BiResNet(cfg, dataset)

        # for open_att + resnet + classify
        self.open_att = BiAttention(cfg.TRAIN.VISION.POOL_DIM, cfg.TRAIN.QUESTION.HID_DIM, cfg.TRAIN.QUESTION.HID_DIM, cfg.TRAIN.ATTENTION.GLIMPSE)
    ...
    def forward(self, v, q, question, a, is_open):
        ...
        output_close = self.close_resnet(v_close, q_close, att_close)
        output_open = self.open_resnet(v_open, q_open, att_open)

        self.open_embeds, self.close_embeds = self.embed_all_answers(answer_prompt)

        return output_close, output_open, a_close, a_open
