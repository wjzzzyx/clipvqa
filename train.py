...
def train(cfg, model, question_model, train_loader, eval_loader, n_unique_close, device, s_opt=None, s_epoch=0):
    ...
    for epoch in range(s_epoch, cfg.TRAIN.N_EPOCH):
        ...
        for i, (v, q, a, answer_type, question_type, phrase_type, answer_target, image_name, question, answer_text) in enumerate(train_loader):
            optim.zero_grad()
            ...
            a = a.to(device)
            if cfg.TRAIN.VISION.AUTOENCODER:
                last_output_close, last_output_open, a_close, a_open, decoder = model(v, q, question, a, answer_target)
            else:
                last_output_close, last_output_open, a_close, a_open = model(v, q, question, a, answer_target)
        ...
        if eval_loader is not None:
            ...
            if eval_score > best_eval_score:
                ...
                model_path = os.path.join(ckpt_path, 'best.pth'.format(best_epoch))
    ...
    model_path = os.path.join(ckpt_path, 'last.pth'.format(best_epoch))
    utils.save_model(model_path, model, best_epoch, eval_score, open_score, close_score, optim)


# Evaluation
def evaluate_classifier(model, question_model, dataloader, cfg, n_unique_close, device, logger):
    score = 0
    total = 0
    open_ended = 0. #'OPEN'
    score_open = 0.
    ...
    with torch.no_grad():
        for i,(v, q, a,answer_type, question_type, phrase_type, answer_target, image_name, question, answer_text) in enumerate(dataloader):
            if cfg.TRAIN.VISION.MAML:
                v[0] = v[0].reshape(v[0].shape[0], 84, 84).unsqueeze(1)
                v[0] = v[0].to(device)
            if cfg.TRAIN.VISION.AUTOENCODER:
            ...
            is_open = question_model(q)
            is_open = torch.argmax(is_open, 1)
            if cfg.TRAIN.VISION.AUTOENCODER:
                last_output_close, last_output_open, a_close, a_open, decoder = model.forward(v, q, question, a, is_open)
            else:
                last_output_close, last_output_open, a_close, a_open = model.forward(v, q, question, a, is_open)
